...
#    def selectwordsToIndex(self,text,doc,pagetitle):
    def selectwordsToIndex(self,lines,doc):
      #begin=time.time()
      i=-1
      len_lines=len(lines)
      if(len_lines==1):
      ...
      while(i<len_lines-1):
            ...
            if line.startswith("\{\{infobox") :   #INFOBOX
                ...
                self.add_to_dict(self.find_tokens(line),doc.body)
...
def main():
  ...
  for line in m:
      ...
      for k in mylist:#basically only 1 element in list
          doc=Document()
          if d.selectwordsToIndex(mylist[k],doc) :
